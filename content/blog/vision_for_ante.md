...
> Code snippets in the rest of this article will be written in Ante. Don't worry too much if you're unfamiliar
...
Let's compare this to the equivalent Rust code _(warning: code gore!)_:
...
and testing. Ok, surely we must gain _something_ with all this extra verbosity though? Absolutely - we get more job security through language-enforced obfuscation!
...
A high-level language's strength is often in its expressivity, or how easy it is to express what a programmer wants to write.
...
is that the amount is still likely non-zero (even Haskell has `IORef`). So our high-level shell should be able to express these
patterns as well:
...
One of the most important concepts for high-level code is _abstraction_. Without abstraction, you're wasting
time repeating yourself or worrying about often irrelevant details. It is a common and important-enough tool that I expect everyone
reading this to already be familiar with it. Yet many languages (even high-level ones) fall victim to repeating
themselves implementing features like exceptions, async, or generators all separately instead of part of one abstraction.
...
writing high-level code involving effects (particularly async, although you can argue Java's checked exceptions lead
...
```ante
// A Stream is any type s which has a function called `stream` to emit values of type a
trait Stream s a with
    stream: fn s -> Unit can Emit a

// This trait-object syntax is non-final
map (s: Stream _ a) (f: fn a -> b): Unit can Emit b =
    handle Stream.stream s
    | emit x = emit (f x)
```

What type should `map` accept? Anything with a `stream` method to `Emit` values.
...
something else, but we don't. High-level code's biggest strength is in letting developer's say "I don't care,"
...
There are other cases effect handlers help with as well. For example, enabling developers to cleanly mock their
code without rearchitecting it to pass mock objects everywhere. This isn't impossible without effect handlers
of course. Just like existing low-level languages _can_ make use of high-level patterns,
it isn't about whether it's possible, it's about how _easy_ it is to actually do so.
With traditional mocking, a developer must have mocking in mind and design their functions to use it. With
...
in a low-level style, or vice-versa. Otherwise, the ecosystem would be split in two. Each of the
examples above still translate into code which follows Ante's rules for ownership and borrowing. For example,
...
they don't want pointers at all and just want to clone them manually?
...
type using `MyPtr`? Then just write the type with `MyPtr`.

---

### Shared Mutability

What about the shared mutability example? Doesn't that break borrowing rules?

Well yes, but actually no. It breaks Rust's borrowing rules, but not Ante's. The short of it is that Ante
extend's Rust's borrowing rules to allow shared mutability based on Rust's `Cell<T>` type. You can
...
When we look at the `NonEmptyList` example with each `shared mut` type translated out:
...
main () =
    ...
    head_mut = head.as_mut ()
    head_mut.next := Some tail
    tail_mut = tail.as_mut ()
    tail.next := Some head

    print head  // prints: NonEmptyList 0 (Some (NonEmptyList 1 (Some (NonEmptyList 0 (Some ...)))))
```

We see an `Rc.as_mut` method. Such a method would not be safe in rust where mutable references must be
...
// here instead of a shared one ensures the reference count stays at at least one while the returned
// reference is alive.
...
Ok, ok, but what about effects? Surely they can't be directly represented in a low-level core?

...Alright you got me. Effects aren't represented in a separate, low-level way using existing concepts.
If we wanted to, we _could_ model some effects with just closures:
...
But this only works for effects which resume in a tail position - it won't work for effects like exceptions
or async.

Not all concepts in a language need to have separate representations for shell and core though. Effects
are already fairly nice to use so they'd only need customizability in the even there are requirements they
are unfit for. This depends largely on their runtime representation though which is currently not set
in stone. Some applications may find a simple requirement sufficient that when declaring an effect, one may
specify that it's handler must be tail-resumptive (and thus implementable as a normal closure), but others
may find this too restrictive and may want effects like async with specific representation requirements
for them. Collecting all the different requirements users may have for the representation of effects, and how
to provide for all or most of them is still an open question.
...
My hope is that this article has shined some light not just on Ante's features but more importantly on _why_ it
has these features and _why_ Ante exists at all. I could get into more philosophy e.g.
"simpler is not easier" (see Brainfuck), but I do think that realizing the ideal of high-level shell, low-level
core is Ante's most important aspiration.
